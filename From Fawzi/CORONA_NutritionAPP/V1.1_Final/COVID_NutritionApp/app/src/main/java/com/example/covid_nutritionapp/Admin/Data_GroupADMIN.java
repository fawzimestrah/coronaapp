...
public static  void insertMemberGroup(ArrayList<Data_GroupADMIN> listGroup, String IdUser){
    FirebaseDatabase database = FirebaseDatabase.getInstance();
    DatabaseReference myRef = database.getReference("GroupAdmin");
    Log.e("checked123","insertmembergroup");

    for(int i=0;i< listGroup.size();i++){
        if(listGroup.get(i).checked){
            Log.e("checked1234","i="+i);
            Log.e("checked123",listGroup.get(i).getNameGroup()+" //"+listGroup.get(i).checked);

            // nb item 3emele waj3t rass
          //  listGroup.get(i).setNumberMember(listGroup.get(i).getNumberMember()+1);
//            myRef.child(listGroup.get(i).getKey_Group()).setValue(listGroup.get(i));
//            myRef.child(listGroup.get(i).getKey_Group()).setValue(listGroup.get(i),"numberMember");
