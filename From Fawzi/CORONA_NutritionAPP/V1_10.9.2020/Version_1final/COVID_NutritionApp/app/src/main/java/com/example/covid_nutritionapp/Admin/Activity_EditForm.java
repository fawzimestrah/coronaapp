...
/*                if(listQuest.size()==0){ // if  no  questions
                    Toast.makeText(getApplicationContext(),"no questions ",Toast.LENGTH_LONG).show();
                    next_Quest(nbQues);

                }
  // eza 3melna l addform bas lal form b5la2 ma3a question fadye b7el lmshkle
  */
            }
            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
    ...
    public void createLayoutQuestion(String TypeQuestion){
        ...
        switch (TypeQuestion) {
            ...
            case "CheckBox":
                ...
                valueQuest.setText(listQuest.get(nbQues).getQuestion());
                //done
                //Toast.makeText(getApplicationContext(),"in createlayout radio  "+nbQues+ " / " +valueQuest.getText().toString(),Toast.LENGTH_LONG).show();
